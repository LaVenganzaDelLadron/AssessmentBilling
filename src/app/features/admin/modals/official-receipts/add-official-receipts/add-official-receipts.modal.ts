...
@Component({
  ...
  styleUrl: './add-official-receipts.modal.css'
})
export class AddOfficialReceiptsModalComponent implements OnInit {
  ...
  submit(): void {
    ...
    this.officialReceiptsService.create(data).subscribe({
      next: () => {
      ...
      error: (error: any) => {
        this.isLoading = false;
        this.errorMessage = error?.message || 'Failed to create receipt';
      }
    });
  }
