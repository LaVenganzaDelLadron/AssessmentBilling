...
export class UpdatePaymentModalComponent {
  ...
  open(payment: Payment): void {
    this.selectedId = payment.id || null;
    this.form = { ...payment };
    this.isOpen = true;
    this.errorMessage = '';
    this.successMessage = '';
  ...
  submit(): void {
    ...
    this.adminDataService.updatePayment(this.selectedId, this.form).subscribe({
      next: () => {
      ...
      error: (error) => {
        this.isLoading = false;
        this.errorMessage = error.error?.message || 'Failed to update payment';
