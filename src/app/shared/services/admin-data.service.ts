...
export class AdminDataService {
  ...
  createFeeStructure(data: FeeStructure): Observable<FeeStructure> {
    return this.http.post<FeeStructure>(`${this.apiUrl}/fee-structures`, data);
  }

  updateFeeStructure(id: number, data: FeeStructure): Observable<FeeStructure> {
    return this.http.put<FeeStructure>(`${this.apiUrl}/fee-structures/${id}`, data);
  }

  deleteFeeStructure(id: number): Observable<{ message: string }> {
    return this.http.delete<{ message: string }>(`${this.apiUrl}/fee-structures/${id}`);
  }
  ...
  createEnrollment(data: Enrollment): Observable<Enrollment> {
    return this.http.post<Enrollment>(`${this.apiUrl}/enrollments`, data);
  }

  updateEnrollment(id: number, data: Enrollment): Observable<Enrollment> {
    return this.http.put<Enrollment>(`${this.apiUrl}/enrollments/${id}`, data);
  }
  ...
  updatePayment(id: number, data: Payment): Observable<Payment> {
    return this.http.put<Payment>(`${this.apiUrl}/payments/${id}`, data);
  }

  deletePayment(id: number): Observable<{ message: string }> {
    return this.http.delete<{ message: string }>(`${this.apiUrl}/payments/${id}`);
  }
  ...
  createRefund(data: Refund): Observable<Refund> {
    return this.http.post<Refund>(`${this.apiUrl}/refunds`, data);
  }

  updateRefund(id: number, data: Refund): Observable<Refund> {
    return this.http.put<Refund>(`${this.apiUrl}/refunds/${id}`, data);
  }

  deleteRefund(id: number): Observable<{ message: string }> {
    return this.http.delete<{ message: string }>(`${this.apiUrl}/refunds/${id}`);
  }
}
